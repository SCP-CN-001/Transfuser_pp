...
from tfpp.data_agent import DataAgent
...
class PlanTAgent(DataAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.data = CarlaDataset(root=[], config=self.config, shared_dict=None)
    ...
    def run_step(
        ...
        target_point = (
            torch.tensor(tick_data["target_point"], dtype=torch.float32)
            .to(self.device)
            .unsqueeze(0)
        )
        ...
        route = (
            torch.tensor(route, dtype=torch.float32)[: self.config.num_route_points]
            .to(self.device)
            .unsqueeze(0)
        )

        light_hazard = (
            torch.tensor(tick_data["light_hazard"], dtype=torch.int32)
            .to(self.device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        stop_sign_hazard = (
            torch.tensor(tick_data["stop_sign_hazard"], dtype=torch.int32)
            .to(self.device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        junction = (
            torch.tensor(tick_data["junction"], dtype=torch.int32)
            .to(self.device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        bounding_boxes_padded = torch.zeros(
            (self.config.max_num_bbs, 8), dtype=torch.float32
        ).to(self.device)

        if len(bounding_boxes) > 0:
            # Pad bounding boxes to a fixed number
            bounding_boxes = np.stack(bounding_boxes)
            bounding_boxes = torch.tensor(bounding_boxes, dtype=torch.float32).to(
                self.device
            )
        ...
        speed = (
            torch.tensor(tick_data["speed"], dtype=torch.float32)
            .to(self.device)
            .unsqueeze(0)
        )
        ...
        if self.config.use_controller_input_prediction:
            ...
            pred_angle = (
                -math.degrees(math.atan2(-pred_aim_wp[1], pred_aim_wp[0])) / 90.0
            )
        ...
        if (
            ...
            steer, throttle, brake = self.nets[0].control_pid_direct(
                pred_target_speed, pred_angle, speed, False
            )
        else:
            steer, throttle, brake = self.nets[0].control_pid(
                self.pred_wp, speed, False
            )
