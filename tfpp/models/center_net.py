...
import team_code.models.gaussian_target as g_t
...
class LidarCenterNetHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.heatmap_head = self._build_head(
            config.bb_input_channel, config.num_bb_classes
        )
    ...
    def loss(
        ...
        loss_center_heatmap = (
            self.loss_center_heatmap(
                center_heatmap_pred, center_heatmap_target, reduction="sum"
            )
            / avg_factor
        )
        ...
        loss_wh = (self.loss_wh(wh_pred, wh_target) * pixel_weight).sum() / (
            avg_factor * wh_pred.shape[1]
        )
        loss_offset = (
            self.loss_offset(offset_pred, offset_target) * pixel_weight
        ).sum() / (avg_factor * wh_pred.shape[1])
        loss_yaw_class = (
            self.loss_dir_class(yaw_class_pred, yaw_class_target) * pixel_weight[:, 0]
        ).sum() / avg_factor
        loss_yaw_res = (
            self.loss_dir_res(yaw_res_pred, yaw_res_target) * pixel_weight[:, 0:1]
        ).sum() / avg_factor
        ...
        if not (self.config.lidar_seq_len == 1 and self.config.seq_len == 1):
            loss_velocity = (
                self.loss_velocity(velocity_pred, velocity_target)
                * pixel_weight[:, 0:1]
            ).sum() / avg_factor
            loss_brake = (
                self.loss_brake(brake_pred, brake_target) * pixel_weight[:, 0]
            ).sum() / avg_factor
