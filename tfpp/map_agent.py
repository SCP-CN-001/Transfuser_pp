...
from tfpp.nav_planner import RoutePlanner, extrapolate_waypoint_route
from tfpp.scenario_logger import ScenarioLogger
...
def interpolate_trajectory(
    world_map, waypoints_trajectory, hop_resolution=1.0, max_len=100
):
    """
...
class MapAgent(autonomous_agent.AutonomousAgent):
    ...
    def _init(self, input_data):
        ...
        new_trajectory = interpolate_trajectory(
            self.hd_map, global_plan_world_coord_positions
        )
    ...
    def tick(self, input_data):
        ...
        ego_target_point = t_u.inverse_conversion_2d(
            target_point, result["gps"], result["compass"]
        )
        ...
        ego_local_point = t_u.inverse_conversion_2d(
            local_point, result["gps"], result["compass"]
        )

        result["angle"] = (
            -math.degrees(math.atan2(-ego_local_point[1], ego_local_point[0])) / 90.0
        )
    ...
    def run_step(
        ...
        if (
            ...
            steer, throttle, brake = self.nets[0].control_pid_direct(
                pred_target_speed, tick_data["angle"], gt_velocity
            )
