...
class AutoPilotV2(AutonomousAgent):
    """
    ...
    def get_control(self, input_data):
        ...
        gps = np.array([pos.x, pos.y, pos.z])
        tick_data = {
            "gps": gps,
            "speed": speed,
            "compass": compass,
    ...
    def save(
        ...
        ego_far_node = t_u.inverse_conversion_2d(
            target_point[:2], pos[:2], theta
        ).tolist()
        ego_next_far_node = t_u.inverse_conversion_2d(
            next_target_point[:2], pos[:2], theta
        ).tolist()
        ego_aim_point = t_u.inverse_conversion_2d(
            self.target_wp[:2], pos[:2], theta
        ).tolist()
        ...
        for checkpoint in remaining_route:
            dense_route.append(
                t_u.inverse_conversion_2d(checkpoint[:2], pos[:2], theta).tolist()
            )
        ...
        data = {
            ...
            "ego_matrix": self.ego_vehicle.get_transform().get_matrix(),
        }
