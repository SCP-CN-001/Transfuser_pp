    ...
    def __getitem__(self, index):
        ...
        for seq in range(0, num_seq):
            ...
            for i in range(2):
                ...
                bounding_boxes_padded = (
                    torch.tensor(bounding_boxes_padded)
                    .unsqueeze(0)
                    .to(self.device, dtype=torch.float32)
                )
                ...
                if augment_sample:
                    ...
                    if self.args.debug:
                        ...
                        pred_bb_parsed = t_u.plant_quant_to_box(
                            self.config, pred_bb_aug
                        )
                        self.model.visualize_model(
                            ...
                            pred_speed=F.softmax(
                                pred_target_speed_aug.squeeze(0), dim=0
                            ).numpy(),
                            gt_speed=velocity,
                            junction=junction,
                            light_hazard=light_hazard,
            ...
            current_measurement["plant_target_speed"] = (
                pred_target_speed.squeeze(0).cpu().tolist()
            )
            current_measurement["plant_target_speed_aug"] = (
                pred_target_speed_aug.squeeze(0).cpu().tolist()
            )

            current_measurement["plant_route"] = (
                pred_checkpoint.squeeze(0).cpu().tolist()
            )
            current_measurement["plant_route_aug"] = (
                pred_checkpoint_aug.squeeze(0).cpu().tolist()
            )
...
def main():
    ...
    data_dummy = CarlaDataset(root=[], config=config, shared_dict=None)
