...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    ...
    def _get_control(self, input_data, plant=False):
        ...
        if self.save_path is not None:
            waypoint_route_logging = extrapolate_waypoint_route(
                waypoint_route, self.config.route_points
            )
            route_logging = np.array(
                [[node[0][0], node[0][1]] for node in waypoint_route_logging]
    ...
    def save(
        ...
        ego_next_far_node = t_u.inverse_conversion_2d(
        ...
        for checkpoint in remaining_route:
            dense_route.append(
                t_u.inverse_conversion_2d(checkpoint[0], pos, theta).tolist()
            )
    ...
    def _get_brake(self, near_command=None, plant=False):
        ...
        number_of_future_frames = int(
            self.config.extrapolation_seconds * self.config.bicycle_frame_rate
        )
        number_of_future_frames_no_junction = int(
            self.config.extrapolation_seconds_no_junction
        ...
        if not plant:
            for vehicle in vehicles:
                if vehicle.id == self._vehicle.id:
                    continue
                if (
                    vehicle.get_location().distance(ego_vehicle_location)
                    ...
                    if (
                        ...
                        self.vehicle_speed_buffer[vehicle.id][
                            "velocity"
                    ...
                    if self.config.model_interactions:
                        traffic_safety_loc = np.array(
                            [self.config.traffic_safety_box_length, 0.0]
                        )
                        ...
                        traffic_safety_extent = carla.Vector3D(
                            ...
                            * self.config.traffic_safety_box_width_multiplier,
                            vehicle.bounding_box.extent.z,
                        )
                        traffic_safety_box = carla.BoundingBox(
                            center_safety_box, traffic_safety_extent
                        )
                    ...
                    for i in range(number_of_future_frames):
                        if not self.junction and (
                            i > number_of_future_frames_no_junction
                        ):
                            break
                        ...
                        if self.config.model_interactions and (
                            self.check_obb_intersection(
                                traffic_safety_box, bounding_boxes_back[i]
                            )
                            or self.check_obb_intersection(
                                traffic_safety_box, bounding_boxes_front[i]
                            )
                        ):
                            traffic_safety_color = carla.Color(255, 0, 0, 255)
                            # Set action to break to model interactions
        ...
        if not plant:
            for i, elem in enumerate(zip(bounding_boxes_front, bounding_boxes_back)):
                ...
                for vehicle_id, traffic_participant in nearby_vehicles.items():
                    ...
                    front_intersect = self.check_obb_intersection(
        ...
        index_future_orientation = int(
            (bremsweg / self.target_speed) * self.config.bicycle_frame_rate
        )

        index_drive_margin = int(
            (1.0 / self.target_speed) * self.config.bicycle_frame_rate
        )
        ...
        for o in range(
            min(index_drive_margin + index_future_orientation, number_of_future_frames)
        ):
        ...
        bounding_box.rotation.yaw = float(
            t_u.normalize_angle_degree(
                np.rad2deg(next_yaw_brake) + bounding_box.rotation.yaw
            )
        )
        ...
        self.traffic_light_hazard = self.ego_agent_affected_by_red_light(
            ego_vehicle_transform, bounding_box
        )
        ...
        return (
            self.vehicle_hazard
            or self.traffic_light_hazard
            or self.walker_hazard
            or self.stop_sign_hazard
        )
    ...
    def forcast_walkers(
        ...
        for walker in walkers:
            if (
                walker.get_location().distance(vehicle_location)
                < self.config.detection_radius
            ):
                ...
                if walker.id in self.walker_past_pos:
                    real_distance = walker_location.distance(
                        self.walker_past_pos[walker.id]
                    )
                ...
                for i in range(number_of_future_frames):
                    ...
                    new_x = walker_location.x + (
                        walker_direction.x
                        * walker_speed
                        * (1.0 / self.config.bicycle_frame_rate)
                    )
    ...
    def ego_agent_affected_by_red_light(self, vehicle_transform, detection_box):
        ...
        for light, center, waypoints in self.list_traffic_lights:
            ...
            for wp in waypoints:
                ...
                center_vehicle = vehicle_transform.transform(
                    self._vehicle.bounding_box.location
                )
                vehicle_bb = carla.BoundingBox(
                    center_vehicle, self._vehicle.bounding_box.extent
                )
                vehicle_bb.rotation = vehicle_transform.rotation

                affects_ego = False
                if self.check_obb_intersection(
                    detection_box, bounding_box
    ...
    def ego_agent_affected_by_stop_sign(
        self, vehicle_transform, vehicle_location, actors, speed, safety_box
    ):
        stop_sign_hazard = False
        self.close_stop_signs.clear()
        stop_signs = self.get_nearby_object(
            vehicle_location, actors.filter("*stop*"), self.config.light_radius
        )
        ...
        for stop_sign in stop_signs:
            ...
            bounding_box_stop_sign = carla.BoundingBox(
                transform_stop_sign.location, stop_sign.trigger_volume.extent
            )
    ...
    def check_obb_intersection(self, obb1, obb2):
        ...
        return not (
            self.get_separating_plane(
                r_pos, obb1.rotation.get_forward_vector(), obb1, obb2
            )
            or self.get_separating_plane(
                r_pos, obb1.rotation.get_right_vector(), obb1, obb2
            )
            or self.get_separating_plane(
                r_pos, obb1.rotation.get_up_vector(), obb1, obb2
            )
            or self.get_separating_plane(
                r_pos, obb2.rotation.get_forward_vector(), obb1, obb2
            )
            or self.get_separating_plane(
                r_pos, obb2.rotation.get_right_vector(), obb1, obb2
            )
            or self.get_separating_plane(
                r_pos, obb2.rotation.get_up_vector(), obb1, obb2
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(
