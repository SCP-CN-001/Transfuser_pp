...
def main():
    ...
    val_set = CarlaDataset(
        root=config.val_data, config=config, shared_dict=shared_dict, rank=rank
    )
        ...
        if (
            ...
            with torch.no_grad():
                if self.config.detect_boxes:
                    pred_bounding_box = self.model.convert_features_to_bb_metric(
                        pred_bounding_box
                    )
                else:
                ...
                visualize_model(
                    ...
                    pred_depth=pred_depth,
                    pred_checkpoint=pred_checkpoint,
                    pred_speed=F.softmax(pred_target_speed, dim=1)
