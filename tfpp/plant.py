...
class PlanT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.cls_emb = nn.Parameter(
            torch.randn(1, self.config.plant_num_attributes + 1)
        )
        ...
        self.obj_emb = nn.ModuleList(
            [
                nn.Linear(self.config.plant_num_attributes, n_embd)
                for _ in range(self.config.plant_object_types)
            ]
        )
    ...
    def forward(
        ...
        other_mask = torch.logical_and(
            walker_mask.logical_not(), car_mask.logical_not()
        )
        other_mask2 = torch.logical_and(
            light_mask.logical_not(), stop_mask.logical_not()
        )
        ...
        embedding = rearrange(
            embedding, "(b o) features -> b o features", b=batch, o=objects
        )
        ...
        embedding = [
            (embedding + obj_embeddings[i]) * masks[i]
            for i in range(self.config.plant_object_types)
        ]
        ...
        route_features = tf_features[
            :, num_boxes_no_route + 1 : route.shape[1] + num_boxes_no_route + 1, :
        ]
        ...
        if self.config.use_controller_input_prediction:
            speed_in = torch.cat(
                [cls_feature, light_hazard, stop_hazard, junction], dim=1
            )
    ...
    def compute_loss(
        ...
        pred_future_bounding_box = [
            rearrange(box, "b o vocab_size -> (b o) vocab_size")
            for box in pred_future_bounding_box
        ]
        future_bounding_box_label = rearrange(
            future_bounding_box_label, "b o vocab_size -> (b o) vocab_size"
        )

        # Compute mean cross entropy loss
        loss_forcast = 0
        for i in range(len(pred_future_bounding_box)):
            loss_forcast += self.loss_forecast(
                pred_future_bounding_box[i], future_bounding_box_label[:, i]
            )
    ...
    def control_pid(self, waypoints, speed, is_stuck):
        ...
        desired_speed = (
            np.linalg.norm(
                waypoints[int(self.config.carla_fps * 0.5)]
                - waypoints[self.config.carla_fps]
            )
            * 2.0
        )
    ...
    def visualize_model(  # pylint: disable=locally-disabled, unused-argument
        ...
        if self.visu_initialized:
            ...
            bev_image = (
                road[:, :, 3:4] * road[:, :, :3] + (1 - road[:, :, 3:4]) * bev_image
            )
        ...
        bev_image = t_u.draw_box(
            bev_image, sample_box, color=(0, 200, 0), pixel_per_meter=16, thickness=4
        )
