...
class GlobalConfig:
    ...
    def __init__(self):
        ...
        self.image_architecture = (
            "regnety_032"  # Image architecture used in the backbone
        )
        self.lidar_architecture = (
            "regnety_032"  # LiDAR architecture used in the backbone
        )
        ...
        self.use_plant_labels = (
            False  # Whether to use the relabeling from plant or the original labels
        )
        ...
        self.creep_duration = (
            20 / self.action_repeat
        )  # Number of frames we will creep forward
        ...
        self.checkpoint_buffer_len = (
            10  # Number of time steps that we use for route consistency
        )
    ...
    def initialize(self, root_dir="", setting="all", **kwargs):
        ...
        for town in self.train_towns:
            ...
            for file in root_files:
                ...
                if (file.find(first_val_town) != -1) or (
                    file.find(second_val_town) != -1
                ):
                    continue
        ...
        for town in self.val_towns:
            ...
            for file in root_files:
                ...
                if (file.find(first_val_town) == -1) and (
                    file.find(second_val_town) == -1
                ):
                    continue
                if not os.path.isfile(os.path.join(self.root_dir, file)):
