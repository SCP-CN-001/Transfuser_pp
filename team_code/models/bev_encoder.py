...
from team_code.models.video_resnet import VideoResNet
from team_code.models.video_swin_transformer import SwinTransformer3D
...
class BevEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.num_features = self.bev_encoder.feature_info.info[bev_start_index + 2][
            "num_chs"
        ]
        ...
        normlaizer = torch.finfo(torch.float32).eps + torch.sum(
            valid_voxels, dim=3, keepdim=False
        ).unsqueeze(1)
        ...
        num_img_features_1 = self.image_encoder.feature_info.info[img_start_index + 1][
            "num_chs"
        ]
        num_img_features_2 = self.image_encoder.feature_info.info[img_start_index + 2][
            "num_chs"
        ]
    ...
    def forward(self, image, lidar):
        ...
        image_features_2 = self.forward_layer_block(
            image_layers, self.image_encoder.return_layers, image_features
        )
        ...
        image = image_features.unsqueeze(
            2
        )  # We add a pseudo dimension for the sample grid function
        ...
        bev_features = F.grid_sample(
            image, grid, align_corners=False, padding_mode="zeros"
        )
