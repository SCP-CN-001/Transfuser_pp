...
from team_code.models.center_net import angle2class


class CarlaDataset(Dataset):  # pylint: disable=locally-disabled, invalid-name
    """
            ...
            for route in routes:
                ...
                for seq in range(
                    ...
                    if estimate_class_distributions:
                        with gzip.open(
                            measurement[-1]
                            + f"/{(seq + self.config.seq_len):04}.json.gz",
                            "rt",
                            encoding="utf-8",
                        ) as f:
    ...
    def __getitem__(self, index):
        ...
        if self.config.use_wp_gru:
            if self.config.use_plant_labels:
                ...
                for i in range(0, self.config.pred_len, self.config.wp_dilation):
                    indices.append(i)
                if augment_sample:
                    data["ego_waypoints"] = np.array(
                        current_measurement["plant_wp_aug"]
                    )[indices]
                else:
                    data["ego_waypoints"] = np.array(current_measurement["plant_wp"])[
                        indices
                    ]
            else:
        ...
        aim_wp = self.augment_target_point(
            aim_wp, y_augmentation=aug_translation, yaw_augmentation=aug_rotation
        )
        data["aim_wp"] = aim_wp
